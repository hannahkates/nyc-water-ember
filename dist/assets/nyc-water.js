...
define('nyc-water/components/bar-chart', ['exports', 'd3'], function (exports, _d) {
  ...
  exports.default = Ember.Component.extend({
    ...
    didInsertElement: function () {
      ...
      var yAxis = _d.default.svg.axis().scale(y).orient("left").ticks(10);

      // add the SVG element
      var svg = _d.default.select("body").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

      var svg2 = _d.default.select("body").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

      var svg3 = _d.default.select("body").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
      ...
      _d.default.json('https://data.cityofnewyork.us/resource/waf7-5gvc.json', function (error, data) {
        ...
        svg.append("g").attr("class", "y axis").call(yAxis).append("text").attr("transform", "rotate(-90)").attr("y", -65).attr("dy", ".71em").style("text-anchor", "end").text("Avg. Gallons Per Person Per Day");
      ...
      _d.default.json('https://data.cityofnewyork.us/resource/waf7-5gvc.json', function (error, data) {
        ...
        svg2.append("g").attr("class", "y axis").call(yAxis).append("text").attr("transform", "rotate(-90)").attr("y", -65).attr("dy", ".71em").style("text-anchor", "end").text("Million Gallons Per Day");
...
if (!runningTests) {
  require("nyc-water/app")["default"].create({"name":"nyc-water","version":"0.0.0+cec96342"});
}
